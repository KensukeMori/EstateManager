...
class StationTable extends Table
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->setDisplayField('stationId');
        $this->setPrimaryKey('stationId');
    }
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ->integer('stationId')
            ->allowEmptyString('stationId', 'create');

        $validator
            ->scalar('stationName')
            ->maxLength('stationName', 20)
            ->requirePresence('stationName', 'create')
            ->allowEmptyString('stationName', false);

        $validator
            ->integer('timeDistance')
            ->requirePresence('timeDistance', 'create')
            ->allowEmptyString('timeDistance', false);
